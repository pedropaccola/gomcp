...
func readPackage(ctx context.Context, eng *store.Store, addr string, fn func(*store.View, address.PkgPath) error) error {
	...
	attempt := func() (bool, error) {
		...
		err := eng.Read(ctx, func(v *store.View) error {
			...
			if extOK && v.HasExternalPackage(ext) {
				found = true
				return fn(v, ext)
			}
			return nil
...
func readSymbol(ctx context.Context, eng *store.Store, addr, key string, fn func(*store.View, store.Symbol, address.PkgPath) error) error {
	return readPackage(ctx, eng, addr, func(v *store.View, pkg address.PkgPath) error {
...
func methodSignatures(v *store.View, pkg address.PkgPath, typeName string) []string {
	var out []string
...
func readFile(ctx context.Context, eng *store.Store, addr, fileName string, fn func(*store.View, address.FilePath, address.PkgPath) error) error {
	return readPackage(ctx, eng, addr, func(v *store.View, pkg address.PkgPath) error {
