...
func TestAddressForms(t *testing.T) {
	...
	if _, _, err := listSymbols(st, testCfg())(context.Background(), nil, ListSymbolsInput{
	...
	if _, _, err := deletePackage(st, testCfg())(context.Background(), nil, DeletePackageInput{
	...
	if _, syms, err := listSymbols(st, testCfg())(context.Background(), nil, ListSymbolsInput{
	...
	if _, _, err := createSymbol(st, testCfg())(context.Background(), nil, CreateSymbolInput{
	...
	if _, _, err := deleteFile(st, testCfg())(context.Background(), nil, DeleteFileInput{
		Deletes: []DeleteFileEntry{{PkgPath: "use", FileName: "alias.go"}},
	}); err != nil {
		t.Errorf("delete_file with (package, file): %v", err)
	}
	if _, _, err := moveFile(st, testCfg())(context.Background(), nil, MoveFileInput{
