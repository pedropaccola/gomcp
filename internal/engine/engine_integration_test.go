...
func TestBootstrapLiveRepo(t *testing.T) {
	...
	if prod.PkgPath != "github.com/pedropaccola/gomcp/internal/engine" {
		t.Errorf("unexpected PkgPath %q", prod.PkgPath)
	}
	if sym, ok := prod.Symbol("Engine.Bootstrap"); !ok || sym.Kind != workspace.KindMethod {
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			...
			for j := 0; j < readIterations; j++ {
				err := e.Read(ctx, func(v *gate.View) error {
	...
	for _, ed := range edits {
		...
		go func(ed editTarget) {
			...
			for j := 0; j < editIterations; j++ {
				if _, err := e.Edit(ctx, func(tx *gate.Tx) error {
